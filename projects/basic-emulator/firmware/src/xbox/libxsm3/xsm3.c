...
uint8_t xsm3_id_data_ms_controller[0x1D] = {
    0x49, 0x4B, 0x00, 0x00, 0x17, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41, 0x41,
    0x41, 0x00, 0x00, 0x80, 0x02, 0x09, 0x12, 0x82, 0x28, 0x03, 0x00, 0x01, 0x01, 0x71};

// static global keys from the keyvault (shared across every retail system)
static const uint8_t xsm3_key_0x1D[0x10] = {0xE3, 0x5B, 0xFB, 0x1C, 0xCD, 0xAD, 0x32, 0x5B,
                                            0xF7, 0x0E, 0x07, 0xFD, 0x62, 0x3D, 0xA7, 0xC4};
static const uint8_t xsm3_key_0x1E[0x10] = {0x8F, 0x29, 0x08, 0x38, 0x0B, 0x5B, 0xFE, 0x68,
                                            0x7C, 0x26, 0x46, 0x2A, 0x51, 0xF2, 0xBC, 0x19};

// retail keys for generating 0x23/0x24 keys from console ID
static const uint8_t xsm3_root_key_0x23[0x10] = {0x82, 0x80, 0x78, 0x68, 0x3A, 0x52, 0x3A, 0x98,
                                                 0x10, 0xF4, 0x0C, 0x12, 0x70, 0x66, 0xDC, 0xBA};

static const uint8_t xsm3_root_key_0x24[0x10] = {0x66, 0x62, 0x1A, 0x78, 0xF8, 0x60, 0x9C, 0x8A,
...
void xsm3_set_identification_data(const uint8_t id_data[0x1D]) {
    ...
    // contains serial number (len: 0xC), unknown (len: 0x2) and the "category node" to use (len:
    // 0x1)
...
void xsm3_generate_kv_keys(const uint8_t console_id[0x8]) {
    ...
    UsbdSecXSM3AuthenticationCrypt(xsm3_root_key_0x23, console_id_hash, 0x10, xsm3_kv_2des_key_1,
                                   1);
    UsbdSecXSM3AuthenticationCrypt(xsm3_root_key_0x24, console_id_hash + 0x4, 0x10,
                                   xsm3_kv_2des_key_2, 1);
}
...
void xsm3_do_challenge_init(uint8_t challenge_packet[0x22]) {
    ...
    UsbdSecXSM3AuthenticationCrypt(xsm3_key_0x1D, challenge_packet + 0x5, 0x18,
                                   xsm3_decryption_buffer, 0);
    ...
    UsbdSecXSM3AuthenticationMac(xsm3_key_0x1E, NULL, challenge_packet + 5, 0x18,
                                 incoming_packet_mac);
    // validate the MAC
    ...
    // and then encrypted - the regular value encrypted with key 1, the swapped value encrypted with
    // key 2
    UsbdSecXSM3AuthenticationCrypt(xsm3_kv_2des_key_1, xsm3_random_console_data, 0x10,
                                   xsm3_random_console_data_enc, 1);
    UsbdSecXSM3AuthenticationCrypt(xsm3_kv_2des_key_2, xsm3_random_console_data_swap, 0x10,
                                   xsm3_random_console_data_swap_enc, 1);
    ...
    UsbdSecXSM3AuthenticationCrypt(xsm3_random_console_data_enc, xsm3_decryption_buffer, 0x20,
                                   xsm3_challenge_response + 0x5, 1);
    // calculate MAC using the encrypted swapped random key and use it to calculate ACR
    UsbdSecXSM3AuthenticationMac(xsm3_random_console_data_swap_enc, NULL,
                                 xsm3_challenge_response + 0x5, 0x20, response_packet_mac);
    // calculate ACR and append to the end of the xsm3_challenge_response
    UsbdSecXSMAuthenticationAcr(xsm3_console_id, xsm3_identification_data, response_packet_mac,
                                xsm3_challenge_response + 0x5 + 0x20);
...
void xsm3_do_challenge_verify(uint8_t challenge_packet[0x16]) {
    ...
    UsbdSecXSM3AuthenticationCrypt(xsm3_random_controller_data, challenge_packet + 0x5, 0x8,
                                   xsm3_decryption_buffer, 0);
    ...
    UsbdSecXSM3AuthenticationMac(xsm3_challenge_init_hash, xsm3_random_console_data,
                                 challenge_packet + 0x5, 0x8, incoming_packet_mac);
    // validate the MAC
    ...
    UsbdSecXSMAuthenticationAcr(xsm3_console_id, xsm3_identification_data,
                                xsm3_random_console_data + 0x8, xsm3_decryption_buffer);
    UsbdSecXSM3AuthenticationCrypt(xsm3_random_console_data_enc, xsm3_decryption_buffer, 0x8,
                                   xsm3_challenge_response + 0x5, 1);
    // calculate the MAC of the encrypted packet and append it to the end
    UsbdSecXSM3AuthenticationMac(xsm3_random_console_data_swap_enc, xsm3_random_console_data,
                                 xsm3_challenge_response + 0x5, 0x8,
                                 xsm3_challenge_response + 0x5 + 0x8);
    // calculate the checksum for the response packet
