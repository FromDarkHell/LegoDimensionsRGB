 ...
 *
 */
enum class TagPadLocation : int8_t
{
    INVALID = 0x00,
 ...
 *        as well as LD-Toypad-Emulator compatible JSON serialization / deserialization.
 */
...
public:
    ...
    ToyTag() : NFCTag(), id(0), padIndex(TagPadLocation::Unplaced), type(ToyTagType::Unknown)
    {
        name[0] = '\0';
...
protected:
    /**
     * @brief Used to fill JSON content for serialization, should be overridden in subclasses to add in extra fields, etc
     *
     * @param doc
     */
    ...
    {
        id = doc["id"] | 0;
        padIndex = static_cast<TagPadLocation>(doc["index"] | 0x00);
 ...
 *
 * Page layout (mirrored from JS createVehicle):
...
public:
    ...
    VehicleTag() : ToyTag(), upgradesP23(0), upgradesP25(0)
    {
        type = ToyTagType::Vehicle;
    }

    VehicleTag(uint16_t vehicleId, const char *vehicleName,
               uint32_t upg23 = 0, uint32_t upg25 = 0)
    ...
    /**
     * @brief Commits all of the stored vehicle data (upgrades/ID + verification) to the underlying NFC byte buffer
     *
     */
    void commitToBuffer()
    {
    ...
    /**
     * @brief Parses the underlying NFC buffer in order to fill the Upgrade/ID fields (also checks the verification byte)
     *
     */
    void readFromBuffer()
    {
 ...
 *
 * The character ID is handled out-of-band by the game via CMD_MODEL / TEA
 * encryption, not written into the NFC byte pages directly. The raw buffer
 * stays mostly zeroed - the game basically only checks that page 26 is 0x00.
 */
